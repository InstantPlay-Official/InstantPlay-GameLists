...
const GAME_UNIT_PRICE = 49;
const HDD_500_PRICE = 849;
const HDD_1TB_PRICE = 1499;
const LIFETIME_PRICE = 599;
...
async function loadOfflineGames() {
  try {
    ...
    offlineGamesList.forEach(item => {
      const key = normalizeName(item.name);
      const isNumericId = /^\d+$/.test(String(item.appid));
      if (key && isNumericId && !numericByName[key]) {
        numericByName[key] = String(item.appid);
      }
    });

    games = offlineGamesList.map((item, index) => {
      const isNumericId = /^\d+$/.test(String(item.appid));
      const idKey = String(item.appid);
      const nameKey = normalizeName(item.name);

      if (seenById.has(idKey) || (nameKey && seenByName.has(nameKey))) {
        return null;
      }
      ...
      if (!imageUrl) {
        if (isNumericId) {
          imageUrl = getHighResImage(item.appid);
        } else {
          const key = normalizeName(item.name);
          const mappedId = numericByName[key];
          imageUrl = mappedId ? getHighResImage(mappedId) : `https://via.placeholder.com/1920x1080?text=${encodeURIComponent(item.name)}`;
        }
      }
...
function renderGames(list) {
  ...
  list.forEach(game => {
    ...
    card.innerHTML = `
      ${rankHtml}
      <div class="img-container" onclick="showGameDetails('${game.id}')">
        ${storageBadgeHtml}
        <img src="${imgSrc}" 
...
function flashCart() {
  ...
  if (!btn) return;
  btn.classList.remove("cart-flash");
...
function syncSelectionUI() {
  ...
  bundleCards.forEach(card => {
    const id = card.getAttribute("data-bundle-id");
    if (!id) return;
...
window.toggleBundle = function(bundleId, name, label) {
  ...
  const existingIndex = cart.findIndex(item => String(item.id) === bundleId);
  const card = document.querySelector(`.bundle-card[data-bundle-id="${bundleId}"]`);

  const bundleIds = ["bundle_500gb", "bundle_1tb"];
  cart = cart.filter(item => !bundleIds.includes(String(item.id)));
  bundleIds.forEach(id => {
    const c = document.querySelector(`.bundle-card[data-bundle-id="${id}"]`);
    if (c) c.classList.remove("bundle-selected");
  });

  if (existingIndex === -1) {
    const price = bundleId === "bundle_500gb" ? HDD_500_PRICE : HDD_1TB_PRICE;
    const bundleItem = {
      id: bundleId,
      name: name,
      storageGB: 0,
    ...
    if (card) card.classList.add("bundle-selected");
    flashCart();
    showToast(`${name} added to cart`);
  } else {
    if (card) card.classList.remove("bundle-selected");
  }
...
window.sendToAdmin = function() {
  ...
  const confirmMessage = "When Messenger opens, your order details will already be copied.\nJust paste the text into the chat to send it to the admin.\n\nDo you want to continue?";
  ...
  navigator.clipboard.writeText(fullMessage).then(() => {
    alert("Order details copied to clipboard. When Messenger opens, paste the text into the chat to send it to the admin.");
...
const savedTheme = localStorage.getItem("instantplay_theme") || "dark";
applyTheme(savedTheme);

themeToggle.onclick = () => {
  const current = localStorage.getItem("instantplay_theme") || "dark";
